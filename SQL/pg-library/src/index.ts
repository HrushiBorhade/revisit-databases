...
async function getUserAndAddress(username: string) {
  await client.connect();

  const res = await client.query(
    `SELECT username,email, a.street , a.city, a.country, a.pincode FROM users JOIN addresses AS a ON users.id = a.user_id WHERE username=$1;`,
    [username]
  );
  console.log("res", res);
}
